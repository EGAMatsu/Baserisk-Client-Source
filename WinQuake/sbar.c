...
// PGM 01/19/97 - team color drawing
	...
	{
		Draw_TransPic (0, 0, sb_face_invis_invuln);
		return;
	}
	if (cl.items & IT_QUAD)
	{
		Draw_TransPic (0, 0, sb_face_quad );
		return;
	}
	if (cl.items & IT_INVISIBILITY)
	{
		Draw_TransPic (0, 0, sb_face_invis );
		return;
	}
	if (cl.items & IT_INVULNERABILITY)
	{
		Draw_TransPic (0, 0, sb_face_invuln);
		return;
	}
	...
	if (cl.time <= cl.faceanimtime)
	{
		...
		anim = 0;
	Draw_TransPic (0, 0, sb_faces[f][anim]);
}
